...
data = load('FaceNet_Representation.npz')
...
model_mine.add(Dense(128, kernel_initializer='normal', activation='relu'))
model_mine.add(Dense(1, kernel_initializer='normal', activation="linear"))
# Compile model
opt = kr.optimizers.Adam(lr=1e-3, decay=1e-3 / 200)
model_mine.compile(loss='mean_squared_error', optimizer=opt)
# print(X_train.shape)
# print(Y_train.shape)
history = model_mine.fit(X_train, Y_train, epochs=1000, batch_size=1024)
